...
def main(argv):
    ...
    for name in names:  # for the different experiments
        ...
        for f in meta_dirs:
            ...
            num_parameters = meta_train['num_parameters'].ravel()
            ...
            order = np.argsort(num_parameters)
            fig = plt.figure()
            ax = fig.add_subplot(111)
            ax.plot(num_parameters[order], loss_train[order], label='train')
            ax.plot(num_parameters[order], loss_test[order], label='test')
            ax.set_xscale('log')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_parameters', action='store_true', help='Use the number of parameter as abscisse')
